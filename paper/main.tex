...
Section~\ref{sec:norelay} presents a session takeover requiring no relay;
...
This method is applicable to many other contactless technologies, including MIFARE Plus and DESFire.
...
Quite interestingly, the earliest account of an asymmetric faulting of a key stored in EEPROM appears in Section 3 of the seminal paper introducing Differential Fault Analysis~\cite{biham_dfa_1997}.
...
Figure~\ref{fig:freq_distrib_ulcg} illustrates this issue: in 10\,000 requests, all LFSR indexes of the observed nonce values fell in the gray zone of the graph and most of them highly concentrated in an even smaller portion of about $2^{7}$ values\footnote{Under ideal conditions --- running the loop in the reader itself, keeping some distance to the reader and not resetting the card between attempts --- we could obtain the exact same nonce more than 98~\% of the time.}.
...
Later, we found a single instance of its deployment in hospitality.
...
\section{Session Takeover Without a Relay}
\label{sec:norelay}

To take over an authenticated session with a tag, the relay attacks described in Sections~\ref{sec:relay} and~\ref{sec:aesrelay} are straightforward in principle but require two communicating devices. This section presents a proof-of-concept single-device variant, implemented as a Proxmark3 RDV4 standalone mode, that eliminates the need for a second relay device entirely by exploiting the physics of near-field coupling.

The idea is to let the tag be authenticated by a legitimate reader, and then take control of the tag with our own reader, without ever interrupting the tag's power supply, leaving it in a continuous authenticated state.

To the best of our knowledge, previous attempts to maintain a tag in an active state while transferring control from one reader to another have always failed: when two readers operate simultaneously at nearly the same carrier frequency, their fields interfere to produce a low-frequency beat of constructive and destructive interference that momentarily deprives the tag of power.

In contrast, our approach leverages a careful positioning and the Proxmark3 RDV4's powerful 9~V antenna driver to create a dominant RF field that can effectively overcome the legitimate reader's field once the tag is authenticated. This allows the Proxmark3 to maintain continuous power to the tag and preserve its authenticated state without needing to switch modes or rely on precise timing.

The technique proceeds as follows.

\begin{itemize}
    \item The Proxmark3 is placed in close contact with a MIFARE Ultralight~C or Ultralight~AES tag and operates as a passive sniffer.
    \item The tag and Proxmark3 are slowly brought together toward the legitimate reader, with the Proxmark3 held directly behind the tag, monitoring the RF field and waiting for a complete authentication exchange between the reader and the tag.
    \item Once the Proxmark3 detects the tag's final authentication response (\textit{ek(RndA')}), confirming that the tag has successfully transitioned to the \texttt{AUTHENTICATED} state, it immediately activates its own reader field. At this point, the Proxmark3's field dominates and effectively suppresses the reader's field, preventing the reader from issuing further commands to the tag while keeping the tag continuously powered and preserving its state. Field beating still occurs, but the Proxmark3's stronger field ensures that even during destructive interference, the tag remains powered. However, the legitimate reader's residual interference also prevents the Proxmark3 from reliably communicating with the tag at this stage.
    \item We then walk away from the legitimate reader, and the Proxmark3 is now in exclusive control of the tag.
\end{itemize}
...
\begin{minipage}{\linewidth}
    \centering
    \resizebox{\columnwidth}{!}{\input{standalone}}
    \captionsetup{hypcap=false}
    \captionof{figure}{Proxmark3 standalone mode in action.}
    \label{fig:ulc-standalone-mode}
\end{minipage}
\vspace{0.5em}

Figure~\ref{fig:ulc-standalone-mode} shows a pair of live RF captures of the standalone mode in action, recorded with nfc-laboratory~\cite{nfclab} using an Airspy Mini, a SpyVerter, and an HydraNFC antenna.
On the top trace, the succession of legitimate reader commands and tag responses during the authentication phase is visible; as soon as the tag's final response is sent, the Proxmark3 activates its field and the beating pattern of the two competing fields becomes apparent. One additional reader command appears, lost in the interference.
On the bottom trace, after walking away from the reader, the field stabilizes. The Proxmark3 sends the unlock command, the tag acknowledges it, and the field is finally turned off.
The observed beating has a frequency of approximately 180 Hz, indicating both 13.56~MHz crystals differ by about 13~ppm.

This technique has been successfully tested with a Proxmark3 RDV4 against both Ultralight~C and Ultralight~AES tags, using Android phones and hospitality readers as the authenticating reader.
However, other Proxmark3 variants with weaker antenna drivers, such as the Proxmark3 Easy, have not been able to reliably overpower the reader's field, and therefore could not maintain the tag in an authenticated state.

The technique can be applied to other types of relay attacks such as the \emph{suspended commit}~\cite{flynn_2919} against MIFARE DESFire~EV1 and EV2. We demonstrated it in a second Proxmark3 RDV4 standalone mode, by switching from sniffer mode to reader mode right before the commit command CRC.
...
After the initial release of this paper, Connor Milligan and Sam Croley independently implemented GPU support (for the Proxmark3~\cite{cuda_des_brute} and \texttt{hashcat}~\cite{hashcat_des_brute}). This enables near-instantaneous recovery, requiring approximately 2 seconds on a modern consumer laptop.
...
In typical deployments using Ultralight~C with default (unlocked) configurations, recovering a static key is within the capabilities of a determined attacker. Further, these findings highlight both the critical security regression presented by prevalent compatible ICs (single-card key recovery in less than a minute, lock byte tearing) and the need for industry-wide awareness and replacement of compatible ICs with genuine NXP chips.
...
Since the initial release of this paper, NXP has updated the datasheets of MIFARE Ultralight~C, MIFARE Ultralight~AES, and NTAG~22x~DNA to mandate or strongly recommend, where available, countermeasures such as locking the user configuration and the key(s), and enabling \texttt{AUTH\_LIM} and \texttt{SEC\_MSG\_ACT}.
...
\begin{table}[ht]
  ...
  \begin{tabularx}{\textwidth}{|l|X|}
    ...
    \makecell[tl]{{\small \texttt{hf mfu cauth/aesauth}}\\{\small \texttt{--retries/--nocheck/--read0}}} & New options to support optimizations described in Sections~\ref{sec:tearing_calibration}, \ref{sec:onlineoptimize} and~\ref{sec:aes_tearing}\\ \hline
    \makecell[tl]{{\small \texttt{hf mfu cauth}}\\{\small \texttt{--noauth/--collect/--pair}}} & New options to perform authentication with colliding nonces as described in Section~\ref{sec:counterfeit_recovery}\\ \hline
    \makecell[tl]{{\small \texttt{hf mfu sim}}\\{\small \texttt{--1a1/--1a2}}\\{\small \texttt{--1a2-mirror}}} & Add support to inject C/AES authentication card reply, supporting relay attacks as described in Sections~\ref{sec:relay} and \ref{sec:aesrelay} and authentication bypass of Section~\ref{sec:aes_oddity} \\ \hline
    ...
    {\small \texttt{hf\_doegox\_auth0}} & Standalone mode to take over a session and unlock an Ultralight C/AES tag, as presented in Section~\ref{sec:norelay} \\ \hline
    {\small \texttt{hf\_doegox\_commit}} & Standalone mode to take over a session and suspend the commit of a DESFire EV1/EV2, as mentioned in Section~\ref{sec:norelay} \\ \hline
  \end{tabularx}
\end{table}
\pagebreak
\begin{table}[ht]
...
\noindent\textbf{2026-01-21 Revision 1.0}
\begin{itemize}
    \item Initial release
\end{itemize}

\noindent\textbf{2026-08-28 Revision 1.1}\\
Additions:
\begin{itemize}
    \item Support of the Ultralight~AES authentication bypass in the Proxmark3
    \item Proxmark3 standalone mode to unlock an Ultralight C/AES without relay
    \item More USCUID-UL fingerprinting data (\texttt{1A00} with spurious data)
    \item Discovery of a FJ8010 case in hospitality
    \item Reference to Biham and Shamir CRYPTO '97 paper
    \item References to GPU port of counterfeit key recovery by Connor and Sam
    \item Mention that KDF check technique also applies to other technologies such as DESFire
    \item Counterfeit authentication based on collisions: add footnote and Proxmark3 options
    \item Mention updated NXP datasheets
\end{itemize}
Errata:
\begin{itemize}
    \item Remove wrong extrapolation from recovery of 12.625 bits per segment to  50.5 bits per card
    \item Fix typos
\end{itemize}
