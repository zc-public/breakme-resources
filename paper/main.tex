...
Section~\ref{sec:norelay} presents a session takeover without relay;
...
It is worth noting that nothing in this approach is specific to MIFARE Ultralight~C; the same technique applies equally to other contactless technologies, such as MIFARE DESFire.
...
The technique could very probably be applied to other types of relay attacks such as the \emph{suspended commit}~\cite{flynn_2919} against MIFARE DESFire~EV1 and EV2 by switching from sniffer mode to reader mode during the commit command CRC, although we have not tested it yet.
...
After the initial release of this paper, Connor Milligan and Sam Croley independently ported the key recovery to GPU (CUDA~\cite{cuda_des_brute} and Hashcat~\cite{hashcat_des_brute}), bringing it down to insane speeds.
...
\begin{table}[ht]
  ...
  \begin{tabularx}{\textwidth}{|l|X|}
    ...
    \makecell[tl]{{\small \texttt{hf mfu cauth/aesauth}}\\{\small \texttt{--retries/--nocheck/--read0}}} & New options to support optimizations described in Sections~\ref{sec:tearing_calibration} and~\ref{sec:aes_tearing}\\ \hline
