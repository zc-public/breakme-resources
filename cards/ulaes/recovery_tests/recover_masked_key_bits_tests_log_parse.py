...
def plot_data_with_gaussian(data, title, xlabel, discrete=False, quiet=False, range=None):
    ...
    if discrete:
        ...
        plt.xticks(unique_values)  # Ensure only unique values are shown on the X axis
        # Format Y axis to use only integers
        plt.gca().yaxis.set_major_locator(plt.MaxNLocator(integer=True))
    else:
        plt.hist(data, bins=20, alpha=0.6, color='blue', range=range)
        # Format Y axis to use only integers
        plt.gca().yaxis.set_major_locator(plt.MaxNLocator(integer=True))
    ...
    with open(file_path, "r") as file:
        ...
        for line in file:
            ...
            if tears_match:
                ...
                if debug:
                    print(f"Tears: {tears}")
            hd_match = re.search(r"HD=(\d+)", line)
            if hd_match:
...
def plot_stats(parsed_data, blocks, title_suffix="(All Blocks)"):
    ...
    plt.subplot(1, 4, 1)
    plot_data_with_gaussian(hd_values, f"Hamming Distance (HD) {title_suffix}", "HD", discrete=True)
    plt.subplot(1, 4, 2)
...
def plot_stats_row(row_index, nblocks, block_data, global_xmin_xmax, global_ymin_ymax, title_suffix=""):
    ...
    plot_data_with_gaussian(time_values, f"Time Spent {title_suffix}", "Time Spent (seconds)",
                            quiet=True, range=global_xmin_xmax["Time Spent (seconds)"])
