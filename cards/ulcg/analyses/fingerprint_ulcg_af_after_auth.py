...
for i in range(1):
    ...
    cmd = 'hf 14a raw -ck af'+ulc_generate_auth_response(rndb, key=key)
    assert len(cmd) == 49
    p.console(cmd)
    output = p.grabbed_output
    for line in output.split('\n'):
        if "[+] 00" in line:
            Erndap = line[7:7+3*8].replace(' ', '')

    cmd = 'hf 14a raw -c af'+ulc_generate_auth_response(rndb,              key=key)
    # cmd = 'hf 14a raw -c af'+ulc_generate_auth_response(rndb,              key=key, iv2=Erndap)
    # cmd = 'hf 14a raw -c af'+ulc_generate_auth_response(rndb,              key=key, iv2="00"*8)
    # cmd = 'hf 14a raw -c af'+ulc_generate_auth_response(rndb[1:]+rndb[:1], key=key)
    # cmd = 'hf 14a raw -c af'+ulc_generate_auth_response(rndb[1:]+rndb[:1], key=key, iv2=Erndap)
    # cmd = 'hf 14a raw -c af'+ulc_generate_auth_response(rndb[1:]+rndb[:1], key=key, iv2="00"*8)
    # cmd = 'hf 14a raw -c af'+ulc_generate_auth_response(rndb[-1:]+rndb[:-1], key=key)
    # cmd = 'hf 14a raw -c af'+ulc_generate_auth_response(rndb[-1:]+rndb[:-1], key=key, iv2=Erndap)
    # cmd = 'hf 14a raw -c af'+ulc_generate_auth_response(rndb[-1:]+rndb[:-1], key=key, iv2="00"*8)
    # cmd = 'hf 14a raw -c af'+ulc_generate_auth_response("00"*8,            key=key)
    # cmd = 'hf 14a raw -c af'+ulc_generate_auth_response("00"*8,            key=key, iv2=Erndap)
    # cmd = 'hf 14a raw -c af'+ulc_generate_auth_response("00"*8,            key=key, iv2="00"*8)
    assert len(cmd) == 48
    ...
    for line in output.split('\n'):
        ...
        if step == 3 and "Tag |00  " in line:
            ...
            print(line)
            print(i, cmd)
            print(rndb, Erndb, decrypt_raw(bytes.fromhex(Erndb), bytes.fromhex(key)).hex().upper())
            print(output)
    print(output)
